...
export const useLoteData = (loteId: string | string[] | undefined) => {
    ...
    const [currentUserId, setCurrentUserId] = useState<string | null>(null); // 👈 ADICIONADO

    // Verificar se o usuário é admin
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, async (user) => {
            if (user) {
                setCurrentUserId(user.uid); // 👈 SALVANDO O ID DO USUÁRIO
                try {
                ...
                setIsAdmin(false);
                setCurrentUserId(null); // 👈 LIMPA SE NÃO HOUVER USUÁRIO
            }
            ...
            async (coletasSnapshot) => {
                ...
                for (const docSnapshot of coletasSnapshot.docs) {
                    ...
                    coletasDoLote.push({
                        ...
                        coletorId: coletorId || '', // 👈 GARANTE QUE O COLETOR ID É INCLUÍDO PARA FILTRAGEM
                    });
                    
                }

                coletasDoLote.sort((a, b) => {
                    // Garante que a comparação seja feita por data e hora correta, assumindo formato DD/MM/AAAA
    ...
    return {
        ...
        updatingStatus,
        isAdmin,
        currentUserId, // 👈 RETORNANDO O ID DO USUÁRIO
        fetchColaboradores,
        handleStatusChange,
