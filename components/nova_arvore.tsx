...
// -------------------- Interfaces --------------------
interface NovaArvoreModalProps {
  visible: boolean;
  onClose: () => void;
...
const NovaArvoreModal: React.FC<NovaArvoreModalProps> = ({
  visible,
  onClose,
  onSuccess,
  loteId,
  ...
  const handleUseCurrentLocation = async () => {
    ...
    try {
      if (locationPermission !== Location.PermissionStatus.GRANTED) {
        ...
        if (status !== Location.PermissionStatus.GRANTED) {
          Alert.alert('Permissão Negada', 'É necessário permitir o acesso à localização.', [{ text: 'OK' }]);
          return;
        }
        setLocationPermission(status);
      }
      ...
      Alert.alert('Localização Obtida', `Coordenadas: ${latitude.toFixed(6)}, ${longitude.toFixed(6)}`, [{ text: 'OK' }]);

    } catch (error) {
      console.error('Erro ao obter localização:', error);
      Alert.alert('Erro de Localização', 'Não foi possível obter sua localização.', [{ text: 'OK' }]);
    } finally {
  ...
  const validateCurrentStep = (): boolean => {
    ...
    if (currentStep === 1) {
      ...
      if (!formData.estadoSaude.trim()) newErrors.estadoSaude = 'Estado de saúde é obrigatório';
    ...
    if (currentStep === 2) {
      ...
      else if (!validateCoordinates(formData.latitude, formData.longitude)) newErrors.latitude = 'Coordenadas inválidas';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, 'arvores'), arvoreData);
      onSuccess(formData);

      Alert.alert('Sucesso!', 'Árvore cadastrada com sucesso!', [{ text: 'OK', onPress: handleClose }]);
    } catch (error) {
  ...
  const renderStep1 = () => (
    <View style={styles.stepContent}>
      ...
      <Text style={styles.stepSubtitle}>Preencha os dados abaixo para adicionar esta árvore à sua produção</Text>
      ...
      <View style={styles.form}>
        ...
        {/* Estado Saúde */}
        <View style={styles.inputGroup}>
          <Text style={styles.inputLabel}><Text style={styles.span}>* </Text>Estado da Árvore</Text>
          <TouchableOpacity
        ...
        {/* Data Plantio */}
        <View style={styles.inputGroup}>
          <Text style={styles.inputLabel}><Text style={styles.span}>* </Text>Data</Text>
    ...
    <View style={styles.stepContent}>
      ...
      <View style={styles.form}>
        ...
        <View style={styles.locationSection}>
          ...
          {currentLocation && (
            <View style={styles.currentLocationInfo}>
              ...
              {currentLocation.accuracy && <Text style={styles.accuracyText}>Precisão: ±{Math.round(currentLocation.accuracy)}m</Text>}
            </View>
          )}
        </View>
        ...
        <View style={styles.inputGroup}>
          ...
          {(errors.latitude || errors.longitude) && <Text style={styles.errorText}>{errors.latitude || errors.longitude}</Text>}
          ...
          {formData.latitude && formData.longitude && (
            validateCoordinates(formData.latitude, formData.longitude) ? (
              <View style={styles.validationSuccess}>
                <Ionicons name="checkmark-circle" size={16} color="#16a34a" />
                <Text style={styles.validationText}>Coordenadas válidas</Text>
              </View>
            ) : (
        ...
        <View style={styles.modalContent}>
          ...
          <ScrollView style={styles.modalOptions}>
            {estadosSaude.map((estado) => (
              <TouchableOpacity
                key={estado.value}
                style={[styles.modalOption, formData.estadoSaude === estado.value && styles.modalOptionSelected]}
                onPress={() => {
                  updateFormData('estadoSaude', estado.value);
                  setShowEstadoModal(false);
                }}
              >
                <Text style={[styles.modalOptionText, formData.estadoSaude === estado.value && styles.modalOptionTextSelected]}>
                  {estado.label}
                </Text>
                {formData.estadoSaude === estado.value && <Ionicons name="checkmark" size={20} color="#16a34a" />}
              </TouchableOpacity>
            ))}
          </ScrollView>
        </View>
        ...
        <View style={styles.bottomButtons}>
          ...
          >
            {isLoading ? (
              ...
              <Text style={styles.continueButtonText}>{currentStep === 1 ? 'Continuar' : 'Cadastrar'}</Text>
            )}
          </TouchableOpacity>
        </View>
...
export default NovaArvoreModal;